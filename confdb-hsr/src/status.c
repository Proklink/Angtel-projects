...
int add_node_to_list(struct hsr_module *app, struct nl_object *n_obj)
{
	...
	if (!rtnl_link_i2name(link_cache, ifindex, link_name,
			      sizeof(link_name))) {
	...
	jv = json_pack("{s:[{ss}]}", "node_list", "mac-address",
		       hnode_get_mac_address_A(node));
	if (!jv) {
...
int delete_node_from_list(struct hsr_module *app, struct nl_object *n_obj)
{
	...
	ret = cdb_edit_status(app->cdb, CDB_OP_REMOVE, jv, XPATH_ITF
		"[name='%s']/angtel-hsr:hsr/node_list[mac-address='%s']",
...
int add_interface_nodes_to_cache(struct nl_sock *sk, struct hsr_module *app,
				 const char *if_name)
{
	...
	ret = nl_connect(gen_sk, NETLINK_GENERIC);
	if (ret < 0) {
		nl_perror(ret, "Unable to connect socket");
		goto _error;
	}
	...
	ret = hnode_alloc_cache_for_interface(gen_sk, &hnode_cache_fi,
